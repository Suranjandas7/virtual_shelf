...
function fetchCurl(targetUrl, res, hostname) {
  const done = () => { releaseHostRequest(hostname); };

  execFile('curl', ['-sS', '-L', '--output', '-', '-H', 'User-Agent: Mozilla/5.0', targetUrl], {
    timeout: 15000,
    maxBuffer: 5 * 1024 * 1024,
    encoding: null,
  }, (err, stdout) => {
    if (err) {
      console.error(`[proxy] curl error for ${targetUrl}: ${err.message || '(no message)'} code=${err.code || 'none'}`);
      if (!res.headersSent) {
        res.writeHead(502);
        res.end('Proxy error');
      }
...
createServer(async (req, res) => {
  ...
  if (url.pathname === '/proxy' && url.searchParams.get('url')) {
    ...
    const useCurl = hostname === 'www.gutenberg.org';
    queueHostRequest(hostname, () => {
      if (useCurl) fetchCurl(target, res, hostname);
